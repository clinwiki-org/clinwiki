...
export const geocodeStudies = async (payload) => {    
    ...
    const facilityQuery = 'select * from facilities where country=$1 and nct_id in ('+params.join(',')+') ';    
    ...
    for(let i=0;i<facilities.rowCount;i++) {
        try {
            ...
            let results = await query('select id from facility_locations where name=$1 and city=$2 and state=$3 and zip=$4 and country=$5',
            [ facility.name,facility.city,facility.state,facility.zip,facility.country ]);
            
            let facilityLocationId;
            if(results.rowCount === 0) {
                // New location. Create a record.
                let insertResults = await query('insert into facility_locations (name,city,state,zip,country) values ($1,$2,$3,$4,$5) RETURNING id',[
                    facility.name,facility.city,facility.state,facility.zip,facility.country
                ]);
            ...
            if(location.partial_match) {
                ...
                if(zip.partial_match) {
                    await query('update facility_locations set status=$1 where id=$2',['bad',facilityLocationId]);
                }
                else {
                    await query('update facility_locations set latitude=$1, longitude=$2, status=$3 where id=$3',[zip.latitude,zip.longitude,'zip',facilityLocationId]);
                ...
                await query('update facility_locations set latitude=$1, longitude=$2, status=$3 where id=$4',[location.latitude,location.longitude,'good',facilityLocationId]);
                addFacilityToStudyMap(facilityMap, {
                    nct_id: facility.nct_id,
                    name: facility.name,
                    city: facility.city,
...
const findOrCreateByName = async (name) => {
    let locations = await query('select * from locations where name=$1',[name]);
    if(locations.rowCount !== 0) {
        return locations.rows[0];
    }
    const location = await query('insert into locations (name) values ($1) RETURNING *',[name]);
...
const geocodeLocation = async (location) => {
    ...
    if(result) {
        await query('update locations set latitude=$1,longitude=$2,partial_match=$3,location_type=$4,checked=$5 where id=$6',
            [result.geometry.location.lat, result.geometry.location.lng, result.partial_match, result.types, new Date(),location.id]);
    }
    else {
        await query('update locations set partial_match=$1,location_type=$2,checked=$3 where id=$4',
            [true, ['note found'], new Date(), location.id]);

    }
    const updatedLoc = await query("select * from locations where id=$1",[location.id]);
