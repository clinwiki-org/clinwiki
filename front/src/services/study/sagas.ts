...
function* createPageViewHasura(action) {
  try {
    ...
    if (createResponse.data.createPageView.errors === null) {
      ...
      yield put(actions.createPageViewHasuraError(createResponse.message));
    }
  } catch (err) {
    console.log(err);
...
function* updatePageViewHasura(action) {
  try {
    ...
    if (updateResponse.data.update_page_views.returning[0]) {
      ...
      let response = yield call(() => api.fetchPageViewHasura(action.id, url));
      let response2 = yield call(() => api.fetchPageViewsHasura(siteIdObj));
      yield put(actions.fetchPageViewHasuraSuccess(response));
      yield put(actions.fetchPageViewsHasuraSuccess(response2));
      yield put(actions.updatePageViewHasuraSuccess(updateResponse));
    } else {
      yield put(actions.updatePageViewHasuraError(updateResponse.message));
    }
  } catch (err) {
    console.log(err);
...
function* deletePageViewHasura(action) {
  ...
  try {
    ...
    if (id === action.id) {
      let newPageViews = currentPageViewsHasura.filter(pv => pv.id !== id);
...
export default function* userSagas() {
  ...
  yield takeLatest(types.FETCH_SAMPLE_STUDY_HASURA_SEND, getHasuraSampleStudy);
