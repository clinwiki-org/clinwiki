...
function* getSiteProvider(action) {
    ...
    try {
        let response = yield call(() =>
            api.fetchSiteProvider(action.id, action.url)
        );
        if (response) {
...
function* deleteSite(action) {
    ...
    try {
        ...
        if (id === action.id) {
            let newEditorSites = currentSites.editorSites.filter(
                site => site.id !== id
            );
            let newOwnSites = currentSites.ownSites.filter(
                site => site.id !== id
            );
            let newSites = {
                id: currentSites.id,
...
export default function* userSagas() {
    ...
    yield takeLatest(
        types.FETCH_PRESENT_SITE_PROVIDER_SEND,
        getPresentSiteProvider
    );
    yield takeLatest(
