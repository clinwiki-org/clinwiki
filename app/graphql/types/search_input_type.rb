module Types
  class SearchInputType < BaseInputObject
    ...
    argument :page, Int, 'which page of search results we want. for now, use either page and pageSize or none of them', required: false, default_value: 1
    argument :pageSize, Int, 'how many results we want. for now, use either page and pageSize or none of them', required: false, default_value: 25
    argument :sorts, [SortType], 'which fields to sort by', required: false, default_value: [ { :field => 'nct_id' }]
    argument :aggOptionsFilter, String, 'the values in aggs will be filtered by that paramater using a substring match', required: false
